...
abstract: |
  Well constructed chronologies are critical to reliable paleoecological inference.  Analysis that relies strongly on chronologies must take into account both the methods' inherent assumptions and the parameters used in the development of the chronologies.  Hence, a first step in large-scale paleodata syntheses is to review and, as needed, revise chronologies. The selection of a single age-model software helps reduce variability due to differing assumptions among models, but the choice of age model parameters, and assumptions about synthesis data are equally important. This paper uses pollen records and associated chronological controls from the Neotoma Paleoecological Database ([http://neotomadb.org]()) to examine the biases and assumptions made in chronology construction on an aggregate dataset.  We examine the challenges of missing uncertainty estimates in ^210^Pb dates, the use of zero-length hiatuses to account for changing accumulation rates with depth. We also use the exercise to provide some prescriptive guidance for chronology construction in the large-scale re-analysis of records.
  
  One third of age models in Neotoma have default chronologies expressed only in radiocarbon years.  Calibrating interpolated ages results in differences of between 31 and 470 cal yrs BP when compared to new age models. Bacon age models using ^210^Pb dates show a linear relationship between ^210^Pb ages and the calibrated ages, but Bacon calculated uncertainties are much higher than the reported ^210^Pb uncertainty and increase exponentially with age. Establishing biostratigraphic markers and using a hiatus at the point of inflection for sediment accumulation at this time period results in age estimates at settlement that are, on average 44 years younger in the new Bacon models.
  
  Bacon parameters fit to the best model show some general trends.  Accumulation rates show a pattern similar to other studies, with an inflection point at approximately 200 years before present.  Best-fit model section thickness increases with absolute model age, and a framework in which each core is reconstructed multiple times with a grid of parameters for section thickness, memory and accumulation provides a basis for broad-scale reconstruction that accounts for regional and local-scale variability in accumulation patterns for sediment over time.
