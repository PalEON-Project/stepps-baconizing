...
```{r load_libraries, echo=FALSE, message=FALSE, results='hide'}
...
meta.table_short <- meta.table %>% group_by(id) %>% filter(year == max(year)) %>% unique()
...
ggplot(na.omit(meta.table_short), aes(x = year, fill = chron)) + geom_bar() +
