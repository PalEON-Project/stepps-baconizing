...
new.chrons <- new.chrons %>% filter(!core %in% c("Chalko Lake", "Aghnaghak", "Pittsburg Basin", "Lac a l'Ange", "Marl Pond", "Cheyenne Bottoms", "San Agustin Plains"))
...
widen <- function(x) {
  data.frame(x$dataset$site.data[,1:5],
             x$geochron,
             row.names = NULL)
}
...
<!-- This is intended to compare  the uncertainties, from Bacon, for 210Pb dates with unassigned uncertainties, to the uncertainties from bacon for the 210Pb dates for assigned 210Pb dates. The point is, that we need to know whethr interpolation is baised high or low. -->
...
The results here will show a table (or histogram, or plot) of section thicknesses vs total core lengths.  Maybe there's a relationship here?

```{r}

# Read in all the settings files and look for the section thickness, then compare against the cores.
