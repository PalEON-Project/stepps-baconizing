...
ggplot(na.omit(meta.table), aes(x = year, fill = chron)) + geom_bar() +
...
model.recal <- function(max.age, depths){
  # This is plotting out the changes between actual and interpolated dates when ages are directly
  # recalibrated.  It's mostly just to 
...
The importance of generating novel age models with harmonized ideas about the underlying mechanics (so weak) is clear for large-scale data synthesis.  Here we present the results of one such excersise.  In this paper we outline the decision making process around the parameter selection for the age models, we summarize differeces between the original chronologies and the new chronologies, we higlight differences between chronologies generated using two age model types and highlight best practices for chronologies within large-scale synthesis.
