...
author: "Andria Dawson, Simon Goring (co-authored)"
output:
  html_document:
    code_folding: show
    highlight: pygment
...
*[120ch] This paper build new age models using Bacon and then looks at the stuff that happens when we do it.*
...
*There's a lot we don't know about making age models for paleoecological records.  One thing we do know: it's a pain, and most people don't know how to do it, and there's not many resources out there for figuring it out on your own.  We did it, and we're telling you all the decisions we made, so you can make some decisions too.*
...
Recent efforts with the European Pollen Database have focused on standardizing age models across the database [@giesecke2014towards]. These models used the software Clam [@blaauw2010methods], as did efforts with the North American Pollen Database [@blois2011methodological].  n both these cases, models were regenerated for records, although 
...
if ('pubs.rds' %in% list.files('data/output')) {
  pubs <- readRDS('data/output/pubs.rds')
} else {
 pubs <- neotoma::get_publication(all.downloads)
 saveRDS(pubs, file = 'data/output/pubs.rds')
}
...
```{r, plot.chron.change, fig.width=6, echo=FALSE, message=FALSE, results='hide', warning=FALSE}
...
```{r, linear-fake, echo=FALSE, message=FALSE, results='hide', warning=FALSE}

source('R/figures/compare_recalibrations.R')

if ('comp_recal.rds' %in% list.files('data/output')) {
  comp_recal <- readRDS('data/output/comp_recal.rds')
} else {
  comp_recal <- compare_recalibrations()
  saveRDS(comp_recal, file = 'data/output/comp_recal.rds')
}
...
Direct recalibration of ages does occur within the Neotoma ecosystem.  For example, the temporal search function within the Neotoma Explorer (http://apps.neotomadb.org/explorer), Tilia (http://tiliait.org) and the Neotoma API (http://api.neotomadb.org) all use a lookup table that directly recalibrates ages in radiocarbon years.  However, this process results in systematic biases in both synthetic data (Figure 2) and in the actual Neotoma data (Figure 3).

```{r, echo=FALSE, message=FALSE, results='hide', warning=FALSE}

source('R/recalibrate.actual.R', echo = FALSE)

basic <- ggplot(new.chrons, aes(x = age.direct, y = age.lin, color = handle)) + 
            geom_line() +
...
# This code block outputs the age models for each record and oututs 
...
diff <- ggplot(new.chrons, aes(x = age.lin, y = diff)) +
...
The exercise of comparing re-calibrated age models against directly calibrated dates highlights two issues.  The first is the possibility of systematic offsets for large-scale research projects, the second is the need for direct intervention and systematic rules-based chronology construction in synthesis work.  Gieseke et al. [-@giesecke2014towards] undertook a broad-scale reconstruction of European records based on prior reported chronologies in an effort to facilitate broad-scale reconstruction of patterns and processes of vegetation change in Europe [@brewer2016late].  Blois et al. [-@blois2011methodological] undertook the same process for eastern North America.  Each of these were labor intensive and resulted in the construction of multiple new age models.

## Limitions of linear/classical models vs Baysian efforts.

Large scale data analysis across heterogeneous data can be complex, thus the use of harmonized age models, that integrate ideas about the underlying mechanics of the processes linking depth and time become important.  Developments since Blois *et al*. [-@blois2011methodological], and workshops such as the Neotoma Belfast Meeting [@grimm2014working] have begun the process of outlining the methods required to undertake large scale efforts to re-build age models.

Here we present the results of one such excersise.  In this paper we outline the decision making process around the parameter selection for the age models, we summarize differeces between the original chronologies and the new chronologies, we higlight differences between chronologies generated using two age model types and highlight best practices for chronologies within large-scale synthesis.

# Methods

## Chronological Uncertainty

### Dealing with zero ^210^Pb errors

Bacon requires that all chronological markers be associated with defined errors. Historically, some ^210^Pb data entered into Neotoma was entered without error reporting.  Of the 398 ^210^Pb markers in Neotoma, 148 have no error reported.  Binford [-@binford1990calculation] reports *"Ninety-five per cent confidence intervals range from about 1 -- 2 years at 10 years of age, 10 -- 20 at 100 years, and 80 -- 90 at 150 years old."*  Using this assessment we fit a smooth linear function to assign 95% CI intervals for all ^210^Pb dates with missing uncertainty data.  These confidence intervals were then divided by 2 (and the integer ceiling taken) to obtain standard deviations to be used in the Bacon model.
...
grid.arrange(lead_smooth, lead_na, ncol = 1)
...
cultural indicators that is reflected in the pollen records, especially *Ambrosia* [@mcandrews1988human]. This change in the relative abundance of Ambrosia, referred to as the *Ambrosia* rise, can be used to identify a reasonably representative presettlement assemblage that justifiable precedes major land clearance - here the pre-settlement pollen sample is defined to be the sample immediately prior to the first pollen sample that reflects the *Ambrosia* rise. We deal
...
We run bacon, we run BChron and we keep the original model outputs.  We comare them all on aggregate and we compare them for specific biostratigraphic events identified in [@blois2011methodological].
...
The results here will show a table (or histogram, or plot) of section thicknesses vs total core lengths.  Maybe there's a relationship here?  Or not. . . 
...
#  USE BCHRON CALIBRATE TO FIX THE AGES in RCDates
source('R/compare_sediment_ages.r')

ggplot(long_df, aes(x = age_default, y = value)) +
  geom_point(aes(color = variable), alpha = 0.1) +
  scale_color_viridis(discrete = TRUE) +
  coord_cartesian(ylim = c(-2000, 2500), xlim = c(-100, 15000), expand = c(0, 0)) +
  geom_smooth() +
  theme_bw() +
  xlab("Existing Age Model") +
  ylab("Model Offset") +
  theme(axis.title.x = element_text(family = 'serif', 
                            face = 'bold.italic', 
                            size = 18),
