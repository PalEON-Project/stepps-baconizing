...
chron <- sapply(all_downloads, function(x)x$sample.meta$age.type[1])
...
Large scale data analysis across heterogeneous data can be complex, thus the use of harmonized age models, that integrate ideas about the underlying mechanics of the processes linking depth and time become important [@blauuw2011].  Developments since Blois *et al*. [-@blois2011methodological], and workshops such as the PAGES-sponsored **Neotoma Belfast Meeting** (what was this called) [@grimm2014working] have begun the process of outlining the methods required to undertake large-scale efforts to re-build age models.
...
```{r, lead_comparison, results = 'hide', echo = FALSE, warning=FALSE}
...
ggplot(data = lead_out) + 
  geom_point(aes(x = 1950 - age, y = bacon_error1), alpha = 0.7) +
  geom_point(aes(x = 1950 - age, y = e.older), color = 'red', alpha = 0.5) +
  coord_cartesian(xlim=c(2000, 1700), ylim=c(0, 200), expand = c(0,0)) +
  scale_x_reverse()

```

When we examine the Bacon uncertainties around the 210Pb dates 

```{r}
drop_outlier <- lead_out %>% filter(bacon_error1 < 200)

diff <- t.test(drop_outlier$bacon_error1, drop_outlier$e.older, paired = TRUE)

```

We can see from these results that the Bacon estimates are, on average, smaller than the ^210^Pb error estimates, when we remove outliers.  This is perhaps a function of the tight constraint that Bacon adds to the estimates (is it a function of the confidence interval?) because Bacon considers the erors as part of a process, the stacked estimates result in a tighter confidence interval.
