...
Bacon <- function(core="MSB2K", thick=5,
                  depths.file=FALSE, 
                  acc.shape=1.5, acc.mean=20, mem.strength=4, mem.mean=0.7, 
                  hiatus.depths=NA, hiatus.shape=1, hiatus.mean=1000, ask=FALSE, suggest=FALSE)

# ### load data, run bacon and analyse outputs
# Bacon <- function(core="MSB2K", thick=5, prob=0.95, d.min=NA, d.max=NA, d.by=1, 
#                   unit="cm", maxcalc=500, depths.file=FALSE, 
#                   acc.shape=1.5, acc.mean=20, mem.strength=4, mem.mean=0.7, 
#                   hiatus.depths=NA, hiatus.shape=1, hiatus.mean=1000, 
#                   after=.0001, cc=1, cc1="IntCal13", cc2="Marine13", cc3="SHCal13",
#                   cc4="ConstCal", postbomb=0, d.R=0, d.STD=0, 
#                   t.a=3, t.b=4, normal=FALSE, suggest=TRUE, reswarn=c(10,200), 
#                   remember=TRUE, cleanup=TRUE, ask=TRUE, run=TRUE, 
#                   defaults="default_settings.txt", sep=",", dec=".", runname="", 
#                   slump=NA, BCAD=FALSE, ssize=2000, rounded=1, 
#                   th0=c(), burnin=min(200, ssize), MinYr=c(), MaxYr=c(), 
#                   find.round=4, bins=c(), cutoff=.001, plot.pdf=TRUE, 
#                   rotate.axes=FALSE, rev.yr=FALSE, rev.d=FALSE, yr.min=c(), 
#                   yr.max=c(), normalise.dists=TRUE, plot.title=core, 
#                   title.location="topleft", d.lab="Depth", yr.lab=c(), d.res=200, 
#                   yr.res=200, date.res=100, grey.res=100, width=15, 
#                   dark=1, greyscale=function(x) grey(1-x), C14.col=rgb(0,0,1,.35),
#                   C14.border=rgb(0,0,1,.5), cal.col=rgb(0,.5,.5,.35), 
#                   cal.border=rgb(0,.5,.5,.5), range.col=grey(0.5), range.lty="12",
#                   hiatus.col=grey(0.5), hiatus.lty="12", wm.col="red", wm.lty="12",
#                   med.col=NA, med.lty="12", mar=c(3,3,1,1), mgp=c(1.5,.7,.0), 
#                   bty="l")
{ 
  maxcalc=500
  d.min=NA
...
#   ask=TRUE
  ...
  C14.col=rgb(0,0,1,.35)
  C14.border=rgb(0,0,1,.5) 
  cal.col=rgb(0,.5,.5,.35) 
  cal.border=rgb(0,.5,.5,.5) 
  range.col=grey(0.5) 
  range.lty="12"
  hiatus.col=grey(0.5) 
  hiatus.lty="12" 
  ...
  info <- .Bacon.settings(core=core, dets=dets, thick=thick, remember=remember, 
                          d.min=d.min, d.max=d.max, d.by=d.by, depths.file=depths.file, 
                          slump=slump, acc.mean=acc.mean, acc.shape=acc.shape, mem.mean=mem.mean, 
                          mem.strength=mem.strength, hiatus.depths=hiatus.depths, hiatus.mean=hiatus.mean, 
                          hiatus.shape=hiatus.shape, BCAD=BCAD, cc=cc, postbomb=postbomb, cc1=cc1, cc2=cc2, 
                          cc3=cc3, cc4=cc4, unit=unit, normal=normal, t.a=t.a, t.b=t.b, d.R=d.R, d.STD=d.STD, 
                          prob=prob, defaults=defaults, runname=runname, ssize=ssize, bins=bins, dark=dark, 
                          rounded=rounded, MinYr=MinYr, MaxYr=MaxYr, cutoff=cutoff, yr.res=yr.res, after=after, 
                          find.round=find.round)
  info <<- info
  ...
  if (info$K < 2){ stop("Fewer than 2 sections! Decrease thickness and try again.")}
  ans <- "n"
  if(suggest)
    if(length(reswarn)==2)
  ...
  if(.Platform$OS == "windows"){
    ...
    info$prefix <- paste("Cores/", core, "/", core, 
                         runname, "_", info$K, sep="")
    info$prefix <- paste("Cores/", core, "/", core, 
                         runname, "_", info$thick, sep="")
  }
  ...
  prepare <- function()
  {
    ...
    calib.plot(info, date.res=date.res, rotate.axes=rotate.axes, width=width, cutoff=cutoff, rev.d=rev.d, 
               rev.yr=rev.yr, 
               normalise.dists=normalise.dists, C14.col=C14.col, C14.border=C14.border, cal.col=cal.col, 
  ...
  cook <- function()
  {
    ...
    if(.Platform$OS == "windows"){
      suppressWarnings(shell(txt, translate=TRUE)) 
    } else {
      system(txt)
    }
  ...
  serve <- function(plot.pdf=FALSE)
  {
    ...
    par(mar=mar, mgp=mgp, bty=bty)
    agedepth(info, bins=bins, d.res=d.res, yr.res=yr.res, rounded=rounded, 
             rotate.axes=rotate.axes, width=width, 
             normalise.dists=normalise.dists, greyscale=greyscale, 
             maxcalc=maxcalc, C14.col=C14.col, C14.border=C14.border, 
             cal.col=cal.col, cal.border=cal.border, plot.range=plot.range, 
             range.col=range.col, wm.col=wm.col, med.col=med.col, 
             range.lty=range.lty, wm.lty=wm.lty, med.lty=med.lty, 
             hiatus.col=hiatus.col, hiatus.lty=hiatus.lty, yr.lab=yr.lab, 
             d.lab=d.lab, yr.min=yr.min, yr.max=yr.max, rev.d=rev.d, 
             rev.yr=rev.yr, plot.title=plot.title, grey.res=grey.res, 
             date.res=date.res, plot.pdf=FALSE, cleanup=cleanup)
  }
  ...
  #     write.table(matrix(info$d), file=paste("Cores/", core, "/", core, "_bacon_depths.csv", sep=""), col.names=FALSE, row.names=FALSE)
  write.table(matrix(info$d), file=paste(info$prefix, "_bacon_depths.csv", sep=""), col.names=FALSE, row.names=FALSE)
...
agedepth <- function(set=info, d.lab="Depth", 
                     yr.lab=ifelse(set$BCAD, "BC/AD", "cal yr BP"), 
                     d.min=set$d.min, d.max=set$d.max, 
                     d.by=set$d.by, yr.min=c(), yr.max=c(), 
                     bins=max(50, if(exists("set$output")) 2*ceiling(sqrt(nrow(set$output)))), 
                     dark=set$dark, prob=set$prob, rounded=0, d.res=200, 
                     yr.res=200, date.res=100, grey.res=100, rotate.axes=FALSE, 
                     rev.yr=FALSE, rev.d=FALSE, maxcalc=500, width=15, cutoff=.001, plot.range=TRUE, range.col=grey(0.5), 
                     range.lty="12", wm.col="red", wm.lty="12", med.col=NA, med.lty="12", C14.col=rgb(0,0,1,.35), 
                     C14.border=rgb(0,0,1,.5), cal.col=rgb(0,.5,.5,.35), cal.border=rgb(0,.5,.5,.5), hiatus.col=grey(0.5), 
                     hiatus.lty="12", greyscale=function(x) grey(1-x), normalise.dists=TRUE, cc=set$cc, plot.title=set$core, 
                     title.location="topleft", after=set$after, bty="l", mar=c(3,3,1,1), mgp=c(1.5,.7,.0), plot.pdf=FALSE, 
                     model.only=FALSE, cleanup=TRUE)
{
  ...
  if(model.only){
    ...
    layout(matrix(if(is.na(set$hiatus.depths)[1]) c(1:3, rep(4,3)) 
                  else c(1:4, rep(5,4)), nrow=2, byrow=TRUE), heights=c(.3,.7))
  }
  ...
  par(yaxs="r")
  if(rotate.axes) {
    plot(0, type="n", ylim=dlim, xlim=yr.lim, ylab=d.lab, xlab=yr.lab, bty="n") 
  } else {
    plot(0, type="n", xlim=dlim[2:1], ylim=yr.lim, xlab=d.lab, ylab=yr.lab, bty="n")
  }
  
  .depth.ghost(set, rotate.axes=rotate.axes, d.res=d.res, yr.res=yr.res, 
               grey.res=grey.res, dark=dark, greyscale=greyscale, d.min=d.min, 
               d.max=d.max, cleanup=cleanup)
  ...
  print('Made it here!')
  usr <- par( "usr" )
  text(usr[1] - (usr[1] - usr[2])/8, usr[4] - (usr[4] - usr[3])/8, plot.title, cex=1.5)
  ...
  for(i in 1:ncol(th))
  {
    ...
    } else
    {
      ...
      lines(d[h], ranges[h,3], col=wm.col, lty=wm.lty)#col=med.col, lty=med.lty) # median
...
{ 
  ...
  #        }
{
      above <- min(which(set$d > hi))
      below <- max(which(set$d < hi))
      
      if (d > set$d[below] && d < set$d[above])
      {
        start.hiatus <- elbows[,below] + (accs[,below] * (hi - set$d[below]))
        end.hiatus   <- elbows[,above] - (accs[,above] * (set$d[above] - hi))
        ok <- which(end.hiatus > start.hiatus)
        if(d < hi){
          ages[ok] <- elbows[ok,below] + (accs[ok,below] * (d - set$d[below])) 
        } else {
          ages[ok] <- elbows[ok,above] - (accs[ok,above] * (set$d[above] - d))
        }
      }
    }
...
# d.jumps = 0.5
# dees <- seq(0, 140, by=d.jumps)
# plot(c(0,0), xlim=c(0,140), ylim=c(-50,6000), type='n')
# for (dee in dees){
#   ages = Bacon.Age.d(dee, set=info, its=set$output, BCAD=set$BCAD)
#   print(ages)
#   hs   = hist(ages, plot=FALSE)
#   bins = hs$mids
#   dens = hs$dens
#   image(c(dee-(d.jumps/2), dee+(d.jumps/2)), bins, t(dens), 
...
.depth.ghost <- function(set=info, bins=max(50, if(exists("set$output")) 2*ceiling(sqrt(nrow(set$output)))), 
                         d.min=set$d.min, d.max=set$d.max, rotate.axes=FALSE, 
                         d.res=100, yr.res=200, grey.res=100, dark=set$dark, 
                         greyscale=function(x) grey(1-x), cleanup=TRUE)
{
  d <- seq(d.min, d.max, length=d.res)
  #     print(paste0("In depth.ghost; d = ", d))
  ...
  #       }
  #     par(mfrow=c(1,1))
  #     plot(c(0,0), xlim=c(-100, 300), ylim=c(0,1), type='n')
  #     cols = c("black", "red", "blue", "cyan", "pink")#rainbow(5)
  for(i in 1:length(d))
  {
    yrs   <- seq(hists[[i]]$th0, hists[[i]]$th1, length=hists[[i]]$n)
    delta <- yrs[2] - yrs[1]
    #         hst   <- approx(yrs, hists[[i]]$counts, seq(min(yrs), max(yrs), length=yr.res))
    dens <- hists[[i]]$counts/sum(hists[[i]]$counts*delta)
    #         hst  <- approx(yrs, dens, seq(min(yrs), max(yrs), length=yr.res))
    #         if(length(hst$y>0) < 5) # for narrow distributions, go narrow
    #           print("Narrow")
    #           hst <- approx(yrs, hists[[i]]$counts, seq(min(yrs), max(yrs), length=20))
    #         if(set$BCAD)
    ...
    #                 add=TRUE, col=greyscale(gr))
    image(c(d[i]-(d.jumps/2), d[i]+(d.jumps/2)), yrs, t(dens*delta), 
          add=TRUE, col=greyscale(gr))
  }
}
                         ...
                         greyscale=function(x) grey(1-x), cleanup=TRUE)
{
  ...
  for(i in 1:length(d))
  {
    ...
    #         hst   <- approx(yrs, hists[[i]]$counts, seq(min(yrs), max(yrs), length=yr.res))
    dens <- hists[[i]]$counts/sum(hists[[i]]$counts*delta)
    hst  <- approx(yrs, dens*delta, seq(min(yrs), max(yrs), length=yr.res))
    # for narrow distributions, go narrow
    if(length(hst$y>0) < 5) {
      print("Narrow")
      hst <- approx(yrs, hists[[i]]$dens*delta, seq(min(yrs), max(yrs), length=20))
    }
    if(set$BCAD){
      hst$x <- rev(1950-hst$x) # Christ...
      hst$y <- rev(hst$y)
...
proxy.ghost <- function(proxy=1, proxy.lab=c(), proxy.res=200, yr.res=200, grey.res=100, set=info, 
                        bins=max(50, if(exists("set$output")) 2*ceiling(sqrt(nrow(set$output)))), 
                        dark=set$dark, rotate.axes=FALSE, rev.proxy=FALSE, rev.yr=FALSE, plot.wm=FALSE, 
                        wm.col="red", yr.lim=c(), proxy.lim=c(), sep=",", xaxs="i", draw.box="l", 
                        BCAD=set$BCAD, yr.lab=ifelse(BCAD, "BC/AD", "cal yr BP"), transpose=FALSE, 
                        cleanup=TRUE)
{
  if(length(set$Tr)==0)
  ...
  if(rotate.axes)
  {
    print("I HATE BACON 1")
    image(proxyseq, yr.seq, max.counts, xlim=proxy.lim, ylim=yr.lim, col=grey(seq(1,0,length=grey.res)), 
          ylab=yr.lab, xlab=proxy.lab, xaxs=xaxs)
    #         stop("No red line. HA")
    if(plot.wm) lines(proxy[,2], pr.wm.ages, col=wm.col)
  } else
  {
    print("I HATE BACON 2")
    image(yr.seq, proxyseq, t(max.counts), xlim=yr.lim, ylim=proxy.lim, col=grey(seq(1,0,length=grey.res)), 
          xlab=yr.lab, ylab=proxy.lab, xaxs=xaxs)
...
Bacon.hist <- function(d, set=info, bins=max(50, if(exists("set$output")) 2*ceiling(sqrt(nrow(set$output)))), 
                       tmpfile=tempfile(), depthfile=tempfile(), xlab=c(), 
                       xlim=c(), ylab="Frequency", ylim=c(), Plot=TRUE, 
                       yr.res=200, prob=set$prob, hist.col=grey(0.5), 
                       hist.border=grey(.2), range.col="blue", med.col="green", 
                       wmean.col="red", cleanup=TRUE)
{
  write(d, depthfile, 1)
  ...
  if(!is.na(set$hiatus.depths)[1])
  {
    ...
    for(hi in set$hiatus.depths)
      for(i in d)
      {
        ...
        {
          #                   print(which.min(abs(round(d.hists,set$find.round)-round(i,set$find.round))))
          this<-which.min(abs(round(d.hists,set$find.round)-round(i,set$find.round)))
          ...
          if(length(this[!is.na(this)]) > 0)
          {
            ...
            #                       hists[[this]]$density <- newdist$density
            hists <<- hists
          }
        }
      }
  ...
  for(i in 1:length(d))
  {
    ...
    if(length(hst$y[hst$y>0]) < 2)
      hst <- approx(yrs, dens*delta, seq(min(yrs), max(yrs), length=50)) # for very narrow ranges
    rng[i,] <- ranges.hst(cbind(hst$x, hst$y))
  }

# BACON version; suspect bug
#   for(i in 1:length(d))
#   {
#     yrs <- seq(hists[[i]]$th0, hists[[i]]$th1, length=hists[[i]]$n)
#     if(set$BCAD) yrs <- 1950 - yrs
...
calib.plot <- function(set=info, rotate.axes=FALSE, rev.d=FALSE, rev.yr=FALSE, 
                       yr.lim=c(), date.res=100, d.lab="Depth", 
                       yr.lab=ifelse(set$BCAD, "BC/AD", "cal yr BP"), 
                       width=15, cutoff=.001, C14.col=rgb(0,0,1,.5), 
                       C14.border=rgb(0,0,1,.75), cal.col=rgb(0,.5,.5,.5), 
                       cal.border=rgb(0,.5,.5,.75), new.plot=TRUE, 
                       plot.dists=TRUE, normalise.dists=TRUE)
{
  ...
  if(new.plot)
    if(rotate.axes){
      plot(0, type="n", xlim=yr.lim, ylim=dlim[2:1], xlab=yr.lab, ylab=d.lab, main="") 
    } else {
  ...
  if(plot.dists)
    for(i in 1:length(set$calib$probs))
    {
      ...
      o <- order(cal[,1])
      if(normalise.dists){
        cal <- cbind(cal[o,1], width*cal[o,2]/sum(cal[,2])) 
      } else {
...
PlotAccPrior <- function(s, mn, set=info, main="", xlim=c(0, 3*max(mn)), xlab=paste("Acc. rate (yr/", noquote(set$unit), ")", sep=""), 
                         ylab="Density", add=FALSE, legend=TRUE, cex=.9)
{
...
PlotHiatusPrior <- function(s=set$hiatus.shape, mn=set$hiatus.mean, hiatus=set$hiatus.depths, set=info, 
                            xlab="Hiatus size (yr)", ylab="Density", main="", xlim=c(0, 3*max(mn)), add=FALSE, legend=TRUE)
{
  o <- order(s, decreasing=TRUE)
  priors <- unique(cbind(s[o],mn[o])[,1:2])
  if(length(priors) == 2)
...
PlotAccPost <- function(set=info, s=set$acc.shape, mn=set$acc.mean, main="", 
                        xlab=paste("Acc. rate (yr/", set$unit, ")", sep=""), 
                        ylab="Frequency")
{ 
  hi <- 2:(set$K-1)
  if(!is.na(set$hiatus.depths)[1])
  ...
  for(i in hi) post <- c(post, set$output[,1+i])
  post <- density(post)
  ...
  if(is.na(set$hiatus.depths))
  {
    ...
    { 
      ...
      if(set$d[whichbelow] > hi && set$d[whichabove] < hi)
        if(d < hi){
          accs <- set$output[,1+1+whichbelow] 
        } else {
