...
Bacon <- function(core="MSB2K", thick=5, prob=0.95, d.min=NA, d.max=NA, d.by=1, unit="cm", maxcalc=500, depths.file=FALSE, 
                  acc.shape=1.5, acc.mean=20, mem.strength=4, mem.mean=0.7, hiatus.depths=NA, hiatus.shape=1, hiatus.mean=1000, 
                  after=.0001, cc=1, cc1="IntCal13", cc2="Marine13", cc3="SHCal13", cc4="ConstCal", postbomb=0, d.R=0, d.STD=0, 
                  t.a=3, t.b=4, normal=FALSE, suggest=TRUE, reswarn=c(10,200), remember=TRUE, cleanup=TRUE, ask=TRUE, run=TRUE, 
                  defaults="default_settings.txt", sep=",", dec=".", runname="", slump=NA, BCAD=FALSE, ssize=2000, rounded=1, 
                  th0=c(), burnin=min(200, ssize), MinYr=c(), MaxYr=c(), find.round=4, bins=c(), cutoff=.001, plot.pdf=TRUE, 
                  rotate.axes=FALSE, rev.yr=FALSE, rev.d=FALSE, yr.min=c(), yr.max=c(), normalise.dists=TRUE, plot.title=core, 
                  title.location="topleft", d.lab="Depth", yr.lab=c(), d.res=200, yr.res=200, date.res=100, grey.res=100, width=15, 
                  dark=1, greyscale=function(x) grey(1-x), C14.col=rgb(0,0,1,.35), C14.border=rgb(0,0,1,.5), cal.col=rgb(0,.5,.5,.35), 
                  cal.border=rgb(0,.5,.5,.5), range.col=grey(0.5), range.lty="12", hiatus.col=grey(0.5), hiatus.lty="12", wm.col="red", wm.lty="12", med.col=NA, med.lty="12", mar=c(3,3,1,1), mgp=c(1.5,.7,.0), bty="l")
  {
    ...
    prepare <- function()
      {
        ...
        calib.plot(info, date.res=date.res, rotate.axes=rotate.axes, width=width, cutoff=cutoff, rev.d=rev.d, rev.yr=rev.yr, 
                   normalise.dists=normalise.dists, C14.col=C14.col, C14.border=C14.border, cal.col=cal.col, cal.border=cal.border)
...
agedepth <- function(set=info, d.lab="Depth", yr.lab=ifelse(set$BCAD, "BC/AD", "cal yr BP"), d.min=set$d.min, d.max=set$d.max, 
                     d.by=set$d.by, yr.min=c(), yr.max=c(), bins=max(50, if(exists("set$output")) 2*ceiling(sqrt(nrow(set$output)))), 
                     dark=set$dark, prob=set$prob, rounded=0, d.res=200, yr.res=200, date.res=100, grey.res=100, rotate.axes=FALSE, 
