...
In the paper we take the position that develping new age models based on recalibrated stratigraphic control points that account for uncertainty is the ideal approach. However, large scale data analysis across heterogeneous data can be complex, thus the use of harmonized age models, that integrate ideas about the underlying mechanics of the processes linking depth and time become important [@blaauw2011flexible]. The Neotoma database [@grimm2008neotoma] contains 1909 global pollen records that can be used for paleoecological analysis.  These records have been obtained from publications that span a time period from  1908 to 2016, with more than half the records coming from before 1983.
...
Methods of understanding an managing radiocarbon uncertainty are well established, however the use of ^210^Pb and biostratigraphic dates are less well understood in the context of sedimentary archives, particularly in large scale synthesis work, where biostratigraphic events may be time-transitive, and uncertainties associated with ^210^Pb may appear to narrow to properly model ages using the Bacon software.
...
<!--Biostratigraphic events sometimes used in paleoecological age models if the cause and timing of the event is well established.
2) Ambrosia rise and other markers (Rumex) often used in eastern North America as signal of EuroAmerican land clearnance (McAndrews, etc. REFS).
This rise is usually identified by visual inspection.
...
For a summary of the settlement horizon elicitation exercise, see the calibration paper [@dawson2016quantifying; @@kujawa2016theeffect].
